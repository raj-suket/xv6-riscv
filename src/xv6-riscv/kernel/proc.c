...
struct bounded_buffer_condition buffer_condition;

struct proc *initproc;

struct barrier barrier_array[10];

struct bounded_buffer_semaphore buffer_sem;
...
void
sleepsemaphore(struct semaphore *s, struct sleeplock *lk)
{
  struct proc *p = myproc();
  uint xticks;
...
void wakeupsemaphore(struct semaphore * s){
  struct proc *p;
  uint xticks;
...
void wakeupbroad(struct cond_t *cv){
    struct proc *p;
  uint xticks;
...
int barrier_alloc(){
  for(int i = 0; i < 10; i++){
    if(barrier_array[i].flag != 1){
      barrier_array[i].flag = 1;
      barrier_array[i].left_processes = 0;
      barrier_array[i].num_processes = 0;
      barrier_array[i].cv.value = 1;
      barrier_array[i].instance = 0;
      return i;
    }
  }
  return -1;
...
void barrier(int instance, int id, int n){
  
  printf("%d: Entered barrier #%d for barrier array id %d\n", myproc()->pid, instance, id);

  acquiresleep(&(barrier_array[id].lk));

  barrier_array[id].num_processes += 1;
  // printf("prc %d cv %d n %d \n", barrier_array[id].num_processes, barrier_array[id].cv.value, n);
  if(barrier_array[id].num_processes == 1){
    barrier_array[id].cv.value = 1;
  }
  if(barrier_array[id].num_processes == n){
    barrier_array[id].cv.value = 0;
    barrier_array[id].num_processes = 0;
    barrier_array[id].instance += 1;
    cond_broadcast(&(barrier_array[id].cv));
  } 
  releasesleep(&(barrier_array[id].lk));

  acquiresleep(&(barrier_array[id].lk));
  while(barrier_array[id].cv.value && (instance == barrier_array[id].instance)) condsleep(&(barrier_array[id].cv), &(barrier_array[id].lk));
  releasesleep(&(barrier_array[id].lk));
  // printf("HI\n");
  // printf("HI\n");
...
void buffer_sem_init(void){
  ...
  buffer_sem.nextc = 0;
  for(int i = 0; i < 20; i++)
    buffer_sem.buffer[i] = -1;
  sem_init(&(buffer_sem.prod), 1);
  sem_init(&(buffer_sem.cons), 1);

  sem_init(&(buffer_sem.empty), 20);
  sem_init(&(buffer_sem.full), 0);
}

void sem_produce(int val){
  sem_wait(&(buffer_sem.empty));
  sem_wait(&(buffer_sem.prod));
  buffer_sem.buffer[buffer_sem.nextp] = val;
  buffer_sem.nextp = (buffer_sem.nextp + 1) % 20;
  sem_post(&(buffer_sem.prod));
  sem_post(&(buffer_sem.full));
}

int sem_consume(void){
  sem_wait(&(buffer_sem.full));
  sem_wait(&(buffer_sem.cons));
  int val = buffer_sem.buffer[buffer_sem.nextc];
  buffer_sem.buffer[buffer_sem.nextc] = - 1;
  buffer_sem.nextc = (buffer_sem.nextc + 1) % 20;
  printf("%d ", val);
  sem_post(&(buffer_sem.cons));
  sem_post(&(buffer_sem.empty));
  return val;
}

void buffer_cond_init(void){
  buffer_condition.head = 0;
  buffer_condition.tail = 0;

  for(int i = 0; i < 20; i++){
    buffer_condition.buffer[i].full = 0;
    buffer_condition.buffer[i].x = -1;
    //
    buffer_condition.buffer[i].inserted.value = 1;
    buffer_condition.buffer[i].deleted.value = 1;
  }
}

void cond_produce(int item){
  int index;
  acquiresleep(&buffer_condition.lock_insert);
      index = buffer_condition.tail;
      buffer_condition.tail = (buffer_condition.tail + 1) % 20;
  releasesleep(&buffer_condition.lock_insert);

  acquiresleep(&(buffer_condition.buffer[index].lock));
    while (buffer_condition.buffer[index].full) condsleep(&(buffer_condition.buffer[index].deleted), &(buffer_condition.buffer[index].lock));
    buffer_condition.buffer[index].x = item;
    buffer_condition.buffer[index].full = 1;
    cond_signal(&(buffer_condition.buffer[index].inserted));
  releasesleep(&(buffer_condition.buffer[index].lock));
}

int cond_consume(){
  int index, item;

  acquiresleep(&buffer_condition.lock_delete);
      index = buffer_condition.head;
      buffer_condition.head = (buffer_condition.head + 1) % 20;
  releasesleep(&buffer_condition.lock_delete);

  acquiresleep(&(buffer_condition.buffer[index].lock));
    while (!buffer_condition.buffer[index].full) condsleep(&(buffer_condition.buffer[index].inserted), &(buffer_condition.buffer[index].lock));
    item = buffer_condition.buffer[index].x;
    buffer_condition.buffer[index].full = 0;
    cond_signal(&(buffer_condition.buffer[index].deleted));
  releasesleep(&(buffer_condition.buffer[index].lock));

  acquiresleep(&buffer_condition.lock_print);
  printf("%d ", item);
  releasesleep(&buffer_condition.lock_print);
  return item;
}
